class card_ui {
  constructor(card_container, compact_slider) {
    ...
    function select_indicator_by_index(index) {
      ...
      Array.from(indicator_list).reduce((pre, cur) => {
        if (cur.dataset.page === index + "") {
          ...
          card_container.fucus_out_all_card();
    ...
    function indicator_event(event) {
      compact_slider.move_to(event.target.dataset.page);
    ...
    function slide_button_event(event) {
      let current_index = compact_slider.current_index;
